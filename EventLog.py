...
class EventLog:
    def __init__(self, df, id_column='id', timestamp_column='timestamp', timestamp_format="%Y-%m-%dT%H:%M:%S%z", sort_and_transform=True):
        self.id_column = id_column
        self.timestamp_column = timestamp_column

        if sort_and_transform:
    ...
    def create_subset(self, mask):
        return EventLog(self.df[mask], id_column=self.id_column, timestamp_column=self.timestamp_column, sort_and_transform=False)

    def groupbyid(self):
        return self.df.groupby(self.id_column)

    def ids(self):
        return self.df[self.id_column]

    def timestamps(self):
        return self.df[self.timestamp_column]        
        
    def static_columns(self):
        return [col for col in self.df.columns.values if ((col not in [self.id_column, self.timestamp_column]) and (not np.any(self.create_mask(mask_type='change_value', column=col))))]
    ...
    def create_mask(self, mask_type, column=None):
        if mask_type == 'start_case':
            return (self.df[self.id_column] != self.df[self.id_column].shift(1))
        elif mask_type == 'end_case':
            return (self.df[self.id_column] != self.df[self.id_column].shift(-1))
        elif mask_type == 'change_value':
            if column is None:
                raise ValueError('No column for mask')
    ...
    def count_events_by_date(self):
        return self.df.groupby(self.df[self.timestamp_column].dt.date)[self.id_column].count()
    
    def count_cases_by_date(self, when='end'):
        op = 'max' if when == 'end' else 'min'
        time = self.df.groupby(self.id_column)[self.timestamp_column].transform(op)
            
        return self.df.groupby(time.dt.date)[self.id_column].nunique()
        ...
        for column in X.columns.values:
            if column != self.id_column:
                ...
                if 'remaining_time' in self.features:
                    ...
                    print(X.groupby(self.id_column)[column].transform('max'))
                    print("------")
                    print(X[column])
...
def compute_workload(log, id_column=None, column=None):
    if type(log) is EventLog:
        id_column = log.id_column if id_column is None else id_column
        column = log.timestamp_column if column is None else column
    
    if (id_column is None) or (column is None):
        raise ValueError('id_column and column cannot be null')

    df_in_out = log.groupby(id_column)[column].agg(['min', 'max'])
    enter_count = df_in_out['min'].value_counts()
    exit_count = df_in_out['max'].value_counts()
    df2 = pd.concat([enter_count, exit_count], axis=1, keys=['enter', 'exit']).fillna(0)
    diff = df2['enter'] - df2['exit']
    return diff.cumsum()
...
class InterCaseFeatures:
    ...
    def _average_interval(workload, x_min, x):
        interval = workload[x_min:x]
        if len(interval) == 0:
            return workload[:x].tail(1)[0]
        else:
    ...
    def transform(self, X):
        ...
        for column in X.columns.values:
            if column != self.id_column:
                workload = compute_workload(X, self.id_column, column)                
                if 'workload' in self.features:
                    X_data.append(X[column].apply(lambda x: workload[:x].tail(1)[0]))
                    X_cols.append('workload_' + column)
                if 'workload_at_start' in self.features:
                    X_data.append(X.groupby(self.id_column)[column].transform(lambda x: workload[:np.min(x).tail(1)[0]]))
                    X_cols.append('workload_at_start_' + column)
                if 'workload_average' in self.features:
                    min_c = X.groupby(self.id_column)[column].transform('min')
                    X_data.append([self._average_interval(workload, min_c.loc[x], X.loc[x, column]) for x in X.index])
                    X_cols.append('workload_average_' + column)
...
def transform_timedeltas(X, unit=pd.Timedelta(seconds=1)):
